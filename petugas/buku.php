...
// Proteksi Halaman: Admin DAN Petugas boleh masuk
...
<head>
    ...
    <title>Pendataan Buku | Sakura Library</title>
    ...
    <style>
        ...
        .btn-sakura { 
            ...
            transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(237, 100, 166, 0.3);
        }
        .btn-sakura:hover { 
            transform: translateY(-2px); box-shadow: 0 6px 20px rgba(237, 100, 166, 0.4); color: white;
        }
        ...
        .table thead th { 
            ...
            font-size: 0.8rem; letter-spacing: 1px; padding: 15px; border: none;
        }
        ...
        /* Badges */
        .badge-stok { 
            font-weight: 700; padding: 8px 12px; border-radius: 10px; font-size: 0.75rem;
        }
        ...
        .btn-action {
            ...
            border-radius: 10px; transition: 0.2s; color: white !important;
        }
...
<nav class="navbar navbar-sakura sticky-top shadow-sm">
    <div class="container">
        <span class="navbar-brand fw-bold" style="color: var(--sakura-dark);">
            <i class="bi bi-flower1"></i> Sakura Library <span class="badge bg-info ms-2" style="font-size: 0.6rem;">PETUGAS</span>
        </span>
        <a href="../<?php echo $_SESSION['role']; ?>/index.php" class="btn btn-sm btn-outline-danger" style="border-radius: 10px;">
...
<div class="container main-content">
    <div class="row align-items-center mb-4">
        <div class="col-md-6">
            <h2 class="fw-bold" style="color: var(--sakura-dark);"><i class="bi bi-journal-bookmark-fill me-2"></i> Pendataan Buku</h2>
            <p class="text-muted">Kelola koleksi buku Digital Library Sakura</p>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="bg-sakura-light p-2 rounded-3 me-3 d-none d-sm-block" style="background: var(--sakura-light); min-width: 45px; text-align: center;">
                                    <i class="bi bi-journal-text fs-4" style="color: var(--sakura-dark);"></i>
                                </div>
                                <div>
                                    <div class="fw-bold text-dark"><?php echo htmlspecialchars($d['judul']); ?></div>
                                    <div class="small text-muted"><i class="bi bi-person"></i> <?php echo htmlspecialchars($d['penulis']); ?></div>
                                    <span class="badge bg-light text-secondary mt-1" style="font-size: 0.65rem; border: 1px solid #eee;">ID: #<?= $d['id_buku']; ?></span>
                                </div>
                            </div>
                        </td>
                        <td class="text-center">
                            <span class="small fw-semibold"><?php echo htmlspecialchars($d['penerbit']); ?></span>
                        </td>
                        <td class="text-center">
                            <span class="badge bg-light text-dark border"><?php echo $d['tahun_terbit']; ?></span>
                        </td>
                        <td class="text-center">
                            <?php if($d['stok'] > 0): ?>
                                <span class="badge-stok stok-aman shadow-sm">
                                    <i class="bi bi-check2-circle me-1"></i> <?= $d['stok']; ?> Unit
                                </span>
                            <?php else: ?>
                                <span class="badge-stok stok-habis shadow-sm">
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-2 justify-content-center">
                                <a href="edit_buku.php?id=<?= $d['id_buku']; ?>" 
                                ...
                                <a href="hapus_buku.php?id=<?= $d['id_buku']; ?>" 
                                   ...
                                   onclick="return confirm('Apakah Anda yakin ingin menghapus buku ini?')" title="Hapus">
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-5">
                            ...
                            <span class="text-muted">Belum ada koleksi buku yang tersedia.</span>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
    ...
    <footer class="text-center mt-5">
        <p class="text-muted small">&copy; 2026 <strong>Sakura Digital Library</strong>. All rights reserved.</p>
    </footer>
</div>
...
<script>
    document.getElementById('searchInput').addEventListener('keyup', function() {
        ...
        for (let i = 0; i < rows.length; i++) {
            let info = rows[i].cells[1].textContent.toLowerCase();
            let penerbit = rows[i].cells[2].textContent.toLowerCase();

            if (info.includes(filter) || penerbit.includes(filter)) {
